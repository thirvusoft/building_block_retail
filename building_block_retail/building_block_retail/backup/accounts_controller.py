...
### Change this Function in accounts_controller.py in erpnext
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	def validate_quantity(child_item, new_data):
		...
		over_delivery_percent = 0
		if not over_delivery_percent:
			over_delivery_percent = frappe.db.get_value("Item", child_item.item_code, "over_delivery_receipt_allowance")
		if not over_delivery_percent:
			over_delivery_percent = frappe.db.get_value("Stock Settings", "Stock Settings", "over_delivery_receipt_allowance")

		over_delivery_qty = (flt(new_data.get("qty"))+1)*flt(over_delivery_percent)/100
		if parent_doctype == "Sales Order" and (flt(new_data.get("qty"))+1+over_delivery_qty) < flt(child_item.delivered_qty):
			
			frappe.throw(_(f"Row: {child_item.get('idx')} Cannot set quantity less than delivered quantity ({flt(new_data.get('qty'), 2)} < {flt(child_item.delivered_qty, 2)})"))
	...
	for d in data:
		...
		if not d.get("docname"):
			...
			if (
				...
				and uom_unchanged
				and date_unchanged
				and parent_doctype != "Sales Order"
			):
				continue
		...
		if parent_doctype == "Sales Order":
			fields_to_update = ["pieces_per_bundle", "pieces_per_sqft", "pieces", "required_sqft"]
			for i in fields_to_update:
				if i in d.keys():
					child_item.update({i:d[i]})
